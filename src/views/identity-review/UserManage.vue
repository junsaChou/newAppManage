    ...
    <el-card shadow="always">
      ...
      >
        <el-form-item label="手机">
          <el-input v-model="listQuery.account" placeholder="手机" />
        </el-form-item>
        <el-form-item label="审核状态">
          <el-select v-model="listQuery.audiState" placeholder="审核状态">
        ...
        <el-form-item label="公司名称">
          <el-input v-model="listQuery.organizeName" placeholder="公司名称" />
        </el-form-item>
      ...
      >
        ...
        <el-table-column prop="id " label="ID" align="center" width="120" sortable />
        ...
        </el-table-column>  手机号码就是账户-->
        <el-table-column prop="account " label="手机号码" align="center" />
        <el-table-column prop="userId " label="经理ID" align="center" />
        ...
        <el-table-column prop="auditState " label="审核状态" align="center" >
         <template slot-scope="scope">{{ scope.row.auditState == '0'?'未通过':'通过' }}</template>
        </el-table-column> 
        <el-table-column label="账户状态" align="center" >
          <template slot-scope="scope">{{ scope.row.userState  == '0'?'冻结':'正常' }}</template>
        </el-table-column> 
        <el-table-column  label="认证状态 " align="center" >
           <template slot-scope="scope">{{ scope.row.faceRecognition  == '0'?'未识别':'已识别' }}</template>
        </el-table-column> 
        <el-table-column prop="organizeName " label="公司名称" align="center" />
        ...
        <el-table-column label="操作" align="center" width="200">
          <template slot-scope="scope">
            ...
            >编辑</el-button>-->
            <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)">审核</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页栏 -->
...
import { apiGetUserAuditList } from "../../api/apilist";
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        account:'',// 账户 ,
        audiState:'',// 审核状态0未通过 1通过 ,
        auditState:'',// 审核状态0未通过 1通过 ,
        createTime:'',// 创建时间 ,
        endTime:'',//
        faceRecognition:'',// 人脸识别0未识别 1识别 ,
        id:'', //id ,
        channelSource :'',//渠道
        // idCard:'',// 身份证号码 ,
        // organizeAddress:'',// 组织地址 ,
        organizeName:'',// 组织名称 ,
        pageIndex:  1,//页码 ,
        pageSize: 20, //每页数据量大小 ,
        realName:'',// 真实姓名 ,
